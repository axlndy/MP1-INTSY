...
int inputPassenger(desType destinations[], int idx, int x, desType inpDest[]){
    ...
    for (int i = 0; i < max && inp != max+1 && inp != 0; i++){
        do{
          dup = 0;
        ...
        } while ( ( (inp < 0 || inp > max+1) || (nPsgr == 0 && inp == max+1) || (dup == 1) ) && (inp != 0) );
        if (inp != max+1){
        	if (inp == 0){
        		for (int j = 0; j < destinations[idx].numPassengers; j++){
    				strcpy(inpDest[x].passengers[j], destinations[idx].passengers[j]);
				}
				return destinations[idx].numPassengers;
			} else {
